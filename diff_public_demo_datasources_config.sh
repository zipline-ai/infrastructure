...
# Downloads public-demo datasource config into a temp directory and diffs it
# against the git-ignored config files currently present locally.

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
current_root="${PUBLIC_DEMO_DATASOURCES_CONFIG_ROOT:-${repo_root}/aws/public-demo-datasources}"
tmp_root="$(mktemp -d)"
...
PUBLIC_DEMO_DATASOURCES_CONFIG_ROOT="${remote_snapshot}" "${repo_root}/pull_public_demo_datasources_config.sh" >/dev/null
...
if [ "${status}" -eq 0 ]; then
  echo "No differences between local public-demo datasource config and remote."
fi
